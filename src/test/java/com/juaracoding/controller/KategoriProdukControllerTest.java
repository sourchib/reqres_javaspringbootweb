...
//            System.out.println(response.getBody().prettyPrint());
    ...
    @Test(priority = 30)
    void findAll(){
        countAllData = kategoriProdukRepo.count();
        Response response ;
        try{
            response = given().
                    header("Content-Type","application/json").
    ...
    void uploadExcel(){
        ...
        try{
            response = given().
                    ...
                    header("accept","*/*").
                    header(AuthControllerTest.AUTH_HEADER,token).
                    multiPart("file",new File(System.getProperty("user.dir")+"/src/test/resources/data-test/kategori-produk.xlsx"),
                            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet").
                    request(Method.POST,"kategoriproduk/upload");

            int intResponse = response.getStatusCode();
            JsonPath jsonPath = response.jsonPath();
            Assert.assertEquals(intResponse,201);
            Assert.assertEquals(jsonPath.getString("message"),"Upload File Berhasil");
            Assert.assertNotNull(jsonPath.getString("data"));
            Assert.assertTrue(Boolean.parseBoolean(jsonPath.getString("success")));
            Assert.assertNotNull(jsonPath.getString("timestamp"));
        }catch (Exception e){
